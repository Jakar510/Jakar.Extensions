    ...
    where TRecord : ITableRecord<TRecord>, IDbReaderMapping<TRecord>
{
    ...
    private readonly string _all       = $"SELECT * FROM {TRecord.TableName};";
    private readonly string _count     = $"SELECT COUNT(*) FROM {TRecord.TableName};";
    private readonly string _deleteAll = $"DELETE FROM {TRecord.TableName};";
    private readonly string _deleteID = $$"""
                                          DELETE FROM {{TRecord.TableName}}
                                          WHERE {{ID}} = '{0}';
                                          """;
    private readonly string _deleteIDs = $$"""
                                           DELETE FROM {{TRecord.TableName}} 
                                           WHERE {{ID}} in ({0});
                                           """;
    ...
    private readonly string _first = $"""
                                      ...
                                      LIMIT 1;
                                      """;
    private readonly string _getWhereIDs = $$"""
                                             SELECT * FROM {{TRecord.TableName}} 
                                             WHERE {{ID}} in ({0});
                                             """;
    private readonly string _getPaged = $$"""
                                          SELECT * FROM {{TRecord.TableName}} 
                                          OFFSET {0}
                                          LIMIT {1};
                                          """;
    private readonly string _getPagedWhere = $$"""
                                               SELECT * FROM {{TRecord.TableName}} 
                                               {0} 
                                               OFFSET {1}
                                               LIMIT {2};
                                               """;
    private readonly string _getPagedWhereCreatedBy = $$"""
                                                        SELECT * FROM {{TRecord.TableName}} 
                                                        WHERE {{CreatedBy}} = '{0}'
                                                        OFFSET {1}
                                                        LIMIT {2};
                                                        """;
    private readonly string _getWhereID = $$"""
                                            SELECT * FROM {{TRecord.TableName}} 
                                            WHERE {{ID}} = '{0}';
                                            """;
    ...
    private readonly string _last = $"""
                                     ...
                                     LIMIT 1
                                     """;
    private readonly string _nextWhereDateCreated = $$"""
                                                      SELECT * FROM {{TRecord.TableName}}
                                                      WHERE ( id = IFNULL((SELECT MIN({{ID}}) FROM {{TRecord.TableName}} WHERE {DateCreated} > '{0}', 0) );
                                                      """;
    private readonly string _nextID_WhereDateCreated = $$"""
    ...
    private readonly string _random = $"""
                                       ...
                                       LIMIT 1;
                                       """;
    private readonly string _randomCount = $$"""
                                             SELECT * FROM {{TRecord.TableName}} 
                                             ORDER BY RANDOM() 
                                             LIMIT {0};
                                             """;
    private readonly string _randomCountWhereCreatedBy = $$"""
                                                           SELECT * FROM {{TRecord.TableName}} 
                                                           WHERE {{CreatedBy}} = '{0}'
                                                           ORDER BY RANDOM() 
                                                           LIMIT {1};
    ...
    public SqlCommand WherePaged( RecordID<UserRecord> id,       int               start,      int count ) => new(string.Format( _getPagedWhereCreatedBy, id.Value.ToString(), start.ToString(), count.ToString() ));
    public SqlCommand WherePaged( int                  start,    int               count ) => new(string.Format( _getPaged, start.ToString(), count.ToString() ));


    public SqlCommand Get( RecordID<TRecord>              id )                                     => new(string.Format( _getWhereID, id.value.ToString() ));
    ...
    public SqlCommand DeleteID( RecordID<TRecord>            id )  => new(string.Format( _deleteID, id.value.ToString() ));
    ...
    public SqlCommand Next( RecordPair<TRecord>   pair ) => new(string.Format( _nextWhereDateCreated,    pair.DateCreated.ToString() ));
    public SqlCommand NextID( RecordPair<TRecord> pair ) => new(string.Format( _nextID_WhereDateCreated, pair.DateCreated.ToString() ));
    ...
    private static Guid                GetValue( RecordID<TRecord>         id )         => id.value;
}
