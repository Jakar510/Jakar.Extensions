    ...
    // IUserPasswordStore<UserRecord>,
{
    ...
    public async Task<UserRecord> FindByIdAsync( string                userId, CancellationToken token ) => await _dbContext.Users.Get( userId, token ) ?? throw new ArgumentException( userId, nameof(userId) );
