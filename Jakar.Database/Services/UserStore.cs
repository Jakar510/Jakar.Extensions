                                ...
                                IUserPhoneNumberStore<UserRecord>
{
    ...
    public async                             Task<IList<UserLoginInfo>> GetLoginsAsync( UserRecord user, CancellationToken token ) => await _dbContext.GetLoginsAsync( user, token ).ToList( token );
    ...
    [ RequiresPreviewFeatures ] public async Task<int> CountCodesAsync( UserRecord user, CancellationToken token ) => (await user.Codes( _dbContext, token ).ToList( token )).Count;
