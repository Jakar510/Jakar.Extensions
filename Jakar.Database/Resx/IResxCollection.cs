...
public interface IResxCollection : IReadOnlyCollection<ResxRowTable>
{
    ...
    public ValueTask Init( IConnectableDb db,         MsSqlDbTable<ResxRowTable> table,       CancellationToken          token                          = default );
    public ValueTask Init( DbConnection   connection, DbTransaction              transaction, MsSqlDbTable<ResxRowTable> table, CancellationToken token = default );
}
