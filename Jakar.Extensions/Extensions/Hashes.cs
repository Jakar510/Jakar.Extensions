...
{
    public static string Hash( this HashAlgorithm hasher, ReadOnlyMemory<byte> data )
    {
        ...
        if ( hasher.TryComputeHash( data.Span, span, out int bytesWritten ) is false ) { throw new InvalidOperationException( nameof(span) ); }
    ...
    public static string Hash_MD5( this ReadOnlyMemory<byte> data )
    {
        using var hasher = MD5.Create();
        return hasher.Hash( data );
    }
    /// <summary> Calculates a file hash using <see cref="SHA1"/> </summary>
    public static string Hash_SHA1( this ReadOnlyMemory<byte> data )
    {
        using var hasher = SHA1.Create();
        return hasher.Hash( data );
    }
    /// <summary> Calculates a file hash using <see cref="SHA256"/> </summary>
    public static string Hash_SHA256( this ReadOnlyMemory<byte> data )
    {
        using var hasher = SHA256.Create();
        return hasher.Hash( data );
    }
    /// <summary> Calculates a file hash using <see cref="SHA384"/> </summary>
    public static string Hash_SHA384( this ReadOnlyMemory<byte> data )
    {
        using var hasher = SHA384.Create();
        return hasher.Hash( data );
    }
    /// <summary> Calculates a file hash using <see cref="SHA512"/> </summary>
    public static string Hash_SHA512( this ReadOnlyMemory<byte> data )
    {
    ...
    public static UInt128 Hash( this ReadOnlySpan<byte> data ) => System.IO.Hashing.XxHash128.HashToUInt128( data );
#endif
}
