...
public static class Json
{
    ...
    {
        public bool GetJsonIsRequired() => propInfo.GetCustomAttribute<JsonRequiredAttribute>() is not null;
        public string GetJsonKey() => propInfo.GetCustomAttribute<JsonPropertyNameAttribute>()
                                             ?.Name ??
                                      propInfo.Name;
    }
    ...
    {
        public JsonNode FromJson()                                    => Validate.ThrowIfNull(JsonNode.Parse(value));
        public TValue   FromJson<TValue>( JsonTypeInfo<TValue> info ) => Validate.ThrowIfNull(JsonSerializer.Deserialize(value, info));
        public TValue FromJson<TValue>()
    ...
    extension( IJsonModel self )
    {
        ...
        public bool Remove( string key, out JsonNode? value )
        {
            JsonObject dict = self.GetAdditionalData();
    ...
    extension( IJsonStringModel self )
    {
        ...
        [Pure] public JsonObject GetAdditionalData()                              => self.AdditionalData?.GetAdditionalData() ?? new JsonObject();
    }
    ...
    extension( IJsonModel self )
    {
        ...
        public TValue? Get<TValue>( string key, JsonTypeInfo<TValue> info )
        {
    ...
    extension( IJsonModel self )
    {
        ...
        public void Add( string                    key, JsonNode? element ) => self.GetAdditionalData()[key] = element;
        public void SetAdditionalData( JsonObject? data )                    => self.AdditionalData = data;
    }
    ...
    {
        public       ValueTask<JsonNode?> FromJson( CancellationToken     token )                            => stream.FromJson(Options, token);
        public async ValueTask<JsonNode?> FromJson( JsonSerializerOptions options, CancellationToken token ) => await JsonNode.ParseAsync(stream, JsonNodeOptions, options.GetJsonDocumentOptions(), token);
        public ValueTask<TValue?> FromJson<TValue>( CancellationToken token )
            where TValue : IJsonModel<TValue> => stream.FromJson(TValue.JsonTypeInfo, token);
        public async ValueTask<TValue?> FromJson<TValue>( JsonTypeInfo<TValue> info, CancellationToken token )
        {
        ...
        public async IAsyncEnumerable<TValue> FromJsonValues<TValue>( [EnumeratorCancellation] CancellationToken token )
            where TValue : IJsonModel<TValue>
        {
            using JsonDocument jsonReader = await JsonDocument.ParseAsync(stream, DocumentOptions, token);
        ...
        public async IAsyncEnumerable<TValue> FromJsonValues<TValue>( JsonTypeInfo<TValue> info, [EnumeratorCancellation] CancellationToken token )
        {
            using JsonDocument jsonReader = await JsonDocument.ParseAsync(stream, DocumentOptions, token);
