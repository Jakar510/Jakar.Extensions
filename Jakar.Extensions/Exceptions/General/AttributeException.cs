...
public sealed class AttributeException : Exception
{
    ...
    {
        ValueStringBuilder sb = new ValueStringBuilder().Append( '"' ).Append( classType.FullName ?? classType.Name ).Append( '.' ).Append( info.Name ).Append( '"' );
        ...
        return sb.Append( '"' ).Append( info.PropertyType.FullName ?? info.PropertyType.Name ).Append( '"' ).ToString();
    }


    private static string GetMessage( Type classType, FieldInfo info )
    {
        ValueStringBuilder sb = new ValueStringBuilder().Append( '"' ).Append( classType.FullName ?? classType.Name ).Append( '.' ).Append( info.Name ).Append( '"' ).Append( " of type " );

        return sb.Append( '"' ).Append( info.FieldType.FullName ?? info.FieldType.Name ).Append( '"' ).ToString();
    }
}
