...
public class WebRequester : IDisposable
{
    ...
    public WebRequester( HttpClient client, Encoding encoding, Uri host )
    {
        _client   = client;
        _encoding = encoding;
        _host     = host;
    ...
    protected virtual WebHandler CreateHandler( Uri url, HttpMethod method, HttpContent value, in CancellationToken token ) => CreateHandler(new HttpRequestMessage(method, url)
                                                                                                                                             ...
                                                                                                                                             token);
    [MethodImpl(MethodImplOptions.AggressiveInlining)] protected virtual WebHandler CreateHandler( HttpRequestMessage request, in CancellationToken token ) => new(_client, request, _encoding, token);
    [MethodImpl(MethodImplOptions.AggressiveInlining)] protected virtual Uri CreateUrl( string                        relativePath ) => new(_host, relativePath);
    [MethodImpl(MethodImplOptions.AggressiveInlining)] protected virtual Uri CreateUrl( Uri                           relativePath ) => new(_host, relativePath);
    ...
    public WebHandler Post( string relativePath, string                      value,   in CancellationToken token ) => Post(relativePath,            new StringContent(value.ToPrettyJson(), _encoding), token);
    public WebHandler Post( string relativePath, BaseClass                   value,   in CancellationToken token ) => Post(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Post( string relativePath, IEnumerable<BaseClass>      value,   in CancellationToken token ) => Post(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Post( string relativePath, BaseRecord                  value,   in CancellationToken token ) => Post(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Post( string relativePath, IEnumerable<BaseRecord>     value,   in CancellationToken token ) => Post(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    ...
    public WebHandler Put( string relativePath, string                      value,   in CancellationToken token ) => Put(relativePath,            new StringContent(value.ToPrettyJson(), _encoding), token);
    public WebHandler Put( string relativePath, BaseClass                   value,   in CancellationToken token ) => Put(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Put( string relativePath, IEnumerable<BaseClass>      value,   in CancellationToken token ) => Put(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Put( string relativePath, BaseRecord                  value,   in CancellationToken token ) => Put(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Put( string relativePath, IEnumerable<BaseRecord>     value,   in CancellationToken token ) => Put(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    ...
    public WebHandler Delete( string relativePath, string                      value,   in CancellationToken token ) => Delete(relativePath,            new StringContent(value.ToPrettyJson(), _encoding), token);
    public WebHandler Delete( string relativePath, BaseClass                   value,   in CancellationToken token ) => Delete(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Delete( string relativePath, IEnumerable<BaseClass>      value,   in CancellationToken token ) => Delete(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Delete( string relativePath, BaseRecord                  value,   in CancellationToken token ) => Delete(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Delete( string relativePath, IEnumerable<BaseRecord>     value,   in CancellationToken token ) => Delete(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    ...
    public WebHandler Patch( string relativePath, string                      value,   in CancellationToken token ) => Patch(relativePath,            new StringContent(value.ToPrettyJson(), _encoding), token);
    public WebHandler Patch( string relativePath, BaseClass                   value,   in CancellationToken token ) => Patch(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Patch( string relativePath, IEnumerable<BaseClass>      value,   in CancellationToken token ) => Patch(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Patch( string relativePath, BaseRecord                  value,   in CancellationToken token ) => Patch(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
    public WebHandler Patch( string relativePath, IEnumerable<BaseRecord>     value,   in CancellationToken token ) => Patch(relativePath,            new JsonContent(value.ToPrettyJson(), _encoding),   token);
