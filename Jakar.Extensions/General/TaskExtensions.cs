...
public static class Tasks
{
    ...
    public static async Task WhenAny(this IEnumerable<Task> tasks) => await Task.WhenAny(tasks).ConfigureAwait(false);
    public static async Task<Task<TResult>> WhenAny<TResult>(this IEnumerable<Task<TResult>> tasks) => await Task.WhenAny(tasks).ConfigureAwait(false);
    public static async Task WhenAll(this IEnumerable<Task> tasks) => await Task.WhenAll(tasks).ConfigureAwait(false);
    public static async Task<TResult[]> WhenAll<TResult>(this IEnumerable<Task<TResult>> tasks) => await Task.WhenAll(tasks).ConfigureAwait(false);
    ...
    public static async Task ForEachParallelAsync(this IEnumerable<Task> source, int? degreeOfParallelism = default, bool continueOnCapturedContext = true)
    {
        async Task AwaitPartition(IEnumerator<Task> partition)
        {
            using (partition)
            {
                while (partition.MoveNext())
                {
                    ...
                    { throw new NullReferenceException(nameof(partition.Current)); }
        ...
        Task tasks = Task.WhenAll(Partitioner.Create(source).GetPartitions(degreeOfParallelism ?? Environment.ProcessorCount).AsParallel().Select(AwaitPartition));

        try
        { await tasks; }
        catch (Exception e)
        {
            if (tasks.Exception is null)
            { throw new NullReferenceException("Unknown error has occurred. Task.Exception is null.", e); }
    ...
    public static async Task ForEachParallelAsync<T>(this IEnumerable<T> source, Func<T, Task> body, int? degreeOfParallelism = default, bool continueOnCapturedContext = true)
    {
        async Task AwaitPartition(IEnumerator<T> partition)
        {
            ...
            {
                while (partition.MoveNext())
                { await body(partition.Current).ConfigureAwait(continueOnCapturedContext); }
            }
        }
    ...
    public static async Task ForEachParallelAsync<T>(this IEnumerable<T> source, Func<T, CancellationToken, Task> body, CancellationToken token, int? degreeOfParallelism = default, bool continueOnCapturedContext = true)
    {
        async Task AwaitPartition(IEnumerator<T> partition)
        {
            ...
            {
                while (partition.MoveNext())
                { await body(partition.Current, token).ConfigureAwait(continueOnCapturedContext); }
            }
        }
