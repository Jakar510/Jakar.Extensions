...
public static class TaskExtensions
{
    ...
    public static async Task ForEachParallelAsync( this IEnumerable<Task> source, int? degreeOfParallelism = default, bool continueOnCapturedContext = true )
    {
        ...
        Task tasks = Task.WhenAll(Partitioner
                                 .Create(source)
    ...
    /// <para>
    ///	Also a debugging improvement: <see href="https://youtu.be/gW19LaAYczI?t=497"/>
    /// </para>
    /// </summary>
    ...
    public static async Task ForEachParallelAsync<T>( this IEnumerable<T> source, Func<T, Task> body, int? degreeOfParallelism = default, bool continueOnCapturedContext = true )
    {
        async Task AwaitPartition( IEnumerator<T> partition )
        {
            using ( partition )
            {
                while ( partition.MoveNext() ) { await body(partition.Current).ConfigureAwait(continueOnCapturedContext); }
            }
        }
                                                      ...
                                                      bool                             continueOnCapturedContext = true )
    {
        async Task AwaitPartition( IEnumerator<T> partition )
        {
            using ( partition )
            {
                while ( partition.MoveNext() ) { await body(partition.Current, token).ConfigureAwait(continueOnCapturedContext); }
            }
        }
    ...
    public static async Task ForEachParallelAsync<T>( this IAsyncEnumerable<T> source,
                                                      Func<T, Task>            action,
                                                      CancellationToken        token,
