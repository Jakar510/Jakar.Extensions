...
public interface IUserRights
{
    ...
    static void Main()
    {
        Permissions<FileRight> rights = Permissions<FileRight>.Create()
                                                              .Add(FileRight.Read)
                                                              .Add(FileRight.Delete);

        foreach ( Permissions<FileRight>.Right r in rights.Rights ) { Console.WriteLine($"{r.Index}: {( r.Value ? "✔" : "✖" )}"); }
