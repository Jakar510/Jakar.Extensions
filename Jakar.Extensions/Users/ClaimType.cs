...
public static class Claims
{
    ...
    public static bool TryParse( this ReadOnlySpan<Claim> claims, out Guid userID, out string userName )
    {
        ...
        if ( Guid.TryParse(claims.FirstOrDefault(IsUserID)
                                ?.Value,
                           out Guid id) )
        {
            userID = id;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)] public static bool IsAuthorized( this IEnumerable<Claim> claims, Guid userID ) => userID != Guid.Empty &&
                                                                                                                                         !string.Equals(claims.FirstOrDefault(static x => x.IsUserID())
                                                                                                                                                             ?.Value,
                                                                                                                                                        userID.ToString(),
