...
public static class HexConverter
{
    ...
    public static unsafe string ToString( scoped ReadOnlySpan<byte> bytes, Casing casing = Casing.Upper )
    {
    ...
    #else
        return string.Create(bytes.Length * 2, ( RosPtr: (IntPtr)( &bytes ), casing ), static ( chars, args ) => EncodeToUtf16(*(ReadOnlySpan<byte>*)args.RosPtr, chars, args.casing));
    #endif
    }
    ...
    {
        return c >= CharToHexLookup.Length
                   ? 0xFF
                   : CharToHexLookup[c];
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)] public static bool IsHexUpperChar( int c ) { return (uint)( c - '0' ) <= 9 || (uint)( c - 'A' ) <= ( 'F' - 'A' ); }

    [MethodImpl(MethodImplOptions.AggressiveInlining)] public static bool IsHexLowerChar( int c ) { return (uint)( c - '0' ) <= 9 || (uint)( c - 'a' ) <= ( 'f' - 'a' ); }
