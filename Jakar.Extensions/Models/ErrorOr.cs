...
public readonly record struct Error( Status? StatusCode, string? Type, string? Title, string? Detail, string? Instance, in StringValues Errors ) : IErrorDetails
{
    ...
    public static Error Create( Status?       status, params string[]     errors )                                                                                        => new(status, null, null, null, null, errors);
    ...
    public static Error Create( Status        status, string?             type, string? title, params string[]     errors )                                               => new(status, type, title, null, null, errors);
    ...
    public static Error Create( Status        status, string?             type, string? title, string?             detail, string? instance, params string[]     errors ) => new(status, type, title, detail, instance, errors);
...
#endif
    public bool HasError { [MethodImpl( MethodImplOptions.AggressiveInlining )] get => Errors?.Length is > 0; }
