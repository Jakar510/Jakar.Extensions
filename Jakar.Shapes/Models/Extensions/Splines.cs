...
public static class Splines
{
    public static ReadOnlyPoint Center<TSpline>( this TSpline self )
        where TSpline : struct, ISpline<TSpline> => new(self.AsValueEnumerable()
                                                            .Sum(static x => x.X) /
                                                        self.Length,
    ...
    public static TSpline Abs<TSpline>( this TSpline self )
        where TSpline : struct, ISpline<TSpline> => TSpline.Create(self.AsValueEnumerable()
                                                                       .Select(static x => x.Abs())
                                                                       .ToArray());
    public static bool IsFinite<TSpline>( this TSpline self )
        where TSpline : struct, ISpline<TSpline> => self.AsValueEnumerable()
