...
{
    public static void AreEqual<T>( this  Assert _, T               expected, T               result ) => Assert.That( result,                           Is.EqualTo( expected ) );
    public static void AreEquals<T>( this Assert _, ReadOnlySpan<T> expected, ReadOnlySpan<T> result ) => Assert.That( result.SequenceEqual( expected ), Is.True );
