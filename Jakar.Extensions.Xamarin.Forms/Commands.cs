...
public class Commands
{
    ...
    public ICommand LoadingCommand( Func<CancellationToken, ValueTask> func, string?  title, string cancel, MaskType mask, Page page ) => new AsyncCommand( async () => await LoadingAsyncTask( func, title, cancel, mask, page ) );
    ...
    public ICommand LoadingCommand( Func<ValueTask> func, string?  title, string cancel, MaskType mask, Page page ) => new AsyncCommand( async () => await LoadingAsyncTask( func, title, cancel, mask, page ) );
    ...
    public async Task LoadingAction( Action func, string? title, string cancel, MaskType mask, Page page )
    {
        ...
        using ( _Prompts.Progress( config ) )
        {
            ...
            catch ( Exception e ) { await _Prompts.HandleExceptionAsync( e, page, cancelSrc.Token ); }
        }
    }
    ...
    public async Task LoadingAsyncTask( Func<CancellationToken, Task> func, string? title, string cancel, MaskType mask, Page page )
    {
        ...
        using ( _Prompts.Progress( config ) )
        {
            try { await func( cancelSrc.Token ); }
            catch ( OperationCanceledException ) { }
            catch ( Exception e ) { await _Prompts.HandleExceptionAsync( e, page, cancelSrc.Token ); }
        }
    }
