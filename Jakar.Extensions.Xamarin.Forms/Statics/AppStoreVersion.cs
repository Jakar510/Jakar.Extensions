...
public static class AppStoreVersion
{
    ...
    public static async Task<bool> VerifyAppStoreVersion<TViewPage>( this Prompts prompts, string newVersionAvailable, string newVersionUpdateNowOrLater, string yes, string no, CancellationToken token = default ) where TViewPage : struct, Enum
    {
        bool isLatest = await IsLatest()
                           .ConfigureAwait( false );
