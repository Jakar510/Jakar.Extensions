...
public static class AppShare
{
    ...
    public static async Task<LocalFile?> OpenOfficeDoc( this Uri link, string shareTitle, MimeType mime, string name )
    {
        ...
        if ( Device.RuntimePlatform == Device.Android ) { await Launcher.OpenAsync(url).ConfigureAwait(false); }
        else { await info.ShareFile(shareTitle, mime.ToString()).ConfigureAwait(false); }
    ...
    {
        Location? location = await LocationManager.GetLocation().ConfigureAwait(false);
        ...
        MediaFile photo = await CrossMedia.Current.TakePhotoAsync(options, token).ConfigureAwait(false);
        return photo;
    }
    ...
    public static async Task<MediaFile> GetPhoto( PickMediaOptions? options = null, CancellationToken token = default )
    {
        ...
        MediaFile photo = await CrossMedia.Current.PickPhotoAsync(options, token).ConfigureAwait(false);
        return photo;
    }
    ...
    public static async Task<MediaFile> TakeVideo( StoreVideoOptions? options = null, CancellationToken token = default )
    {
        ...
        MediaFile photo = await CrossMedia.Current.TakeVideoAsync(options, token).ConfigureAwait(false);
        return photo;
    }

    public static async Task<MediaFile> GetVideo( CancellationToken token = default ) => await CrossMedia.Current.PickVideoAsync(token).ConfigureAwait(false);
    ...
    public static async Task BufferScreenShot() => _ScreenShotBuffer = await TakeScreenShot().ConfigureAwait(false);

    public static async Task<string> GetScreenShot( this FileSystemApi api )
    {
        ReadOnlyMemory<byte> screenShot = await TakeScreenShot().ConfigureAwait(false);
        return await api.WriteScreenShot(screenShot).ConfigureAwait(false);
    }

    public static async Task<ReadOnlyMemory<byte>> TakeScreenShot() => await MainThread.InvokeOnMainThreadAsync(CrossScreenshot.Current.CaptureAsync).ConfigureAwait(false);

    public static async Task<string> WriteScreenShot( this FileSystemApi api, CancellationToken token = default ) => await api.WriteScreenShot(_ScreenShotBuffer.ToArray(), token).ConfigureAwait(false);
    ...
    public static async Task<string> WriteScreenShot( this FileSystemApi api, ReadOnlyMemory<byte> screenShot, CancellationToken token = default )
    {
        ...
        await file.WriteAsync(screenShot, token).ConfigureAwait(false);
