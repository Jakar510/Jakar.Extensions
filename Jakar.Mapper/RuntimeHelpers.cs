...
public static class RuntimeHelpers
{
    ...
    public static unsafe int ConvertLength<TFrom, TTo>( int length ) where TFrom : unmanaged
                                                                     where TTo : unmanaged
    {
        if ( sizeof(TFrom) == sizeof(TTo) ) { return length; }
        else if ( sizeof(TFrom) == 1 ) { return (int)( (uint)length / (uint)sizeof(TTo) ); }
        else
        {
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static IntPtr GetArray2DDataByteOffset<T>() => TypeInfo<T>.array2DDataByteOffset;
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static IntPtr GetArray3DDataByteOffset<T>() => TypeInfo<T>.array3DDataByteOffset;
    ...
    {
        if ( obj is null ) { return (IntPtr)Unsafe.AsPointer(ref data); }
    ...
    {
        if ( obj is null ) { return ref Unsafe.AsRef<T>((void*)offset); }
    ...
    private static bool IsReferenceOrContainsReferences( Type type )
    {
        ...
        if ( Nullable.GetUnderlyingType(type) is { } nullableType ) { type = nullableType; }

        if ( type.GetTypeInfo().IsEnum ) { return false; }

        // Complex struct, recursively inspect all fields
        return type.GetTypeInfo().DeclaredFields.Where(field => !field.IsStatic).Any(field => IsReferenceOrContainsReferences(field.FieldType));
    }
